...
def main() -> None:
    ...
    c_train_all_scaled = scaler.fit_transform(c_train_all)
    np.savez(config.data_dir / "podnn_scaler.npz", mean=scaler.mean_, scale=scaler.scale_)
    print(f"\nTraining ensemble of {N_ENSEMBLE} SCALED PODNN models "
    ...
    for k in range(N_ENSEMBLE):
        mu_tr, c_tr, mu_val, c_val = train_val_split(
            mu_train_f, c_train_all_scaled, val_frac=VAL_FRAC, seed=k)
    ...
    def ensemble_predict(mu_f: np.ndarray) -> np.ndarray:
        ...
        scaled_preds = np.mean([m.predict(mu_f) for m in ensemble], axis=0)
        return scaler.inverse_transform(scaled_preds)
    ...
    sweep_sizes = np.array([train_params.shape[0]])
    sweep_mean_u, sweep_mean_p = [summary['mean_l2_u']], [summary['mean_l2_p']]
    ...
    # --- 7. Save ensemble and metrics ----------------------------------
    for k, member in enumerate(ensemble):
        member.save(config.data_dir / f"podnn_scaled_ensemble_member_{k}.pt")
    np.savez_compressed(
        config.data_dir / "podnn_scaled_ensemble_meta.npz",
